...
var boxUsed = [true, false, false, false];//存储vedio是否被使用
var maxCALLERS = 3;//最大调用人数-1
...
function messageListener(easyrtcid, msgType, content) {
	...
	switch (msgType) {
		...
		case 'reply_userinfo'://接收消息
			...
			videoCallHandle.apply(meetingOwner, ['userinfo', userInfo]);
			break;
	}
}
...
function initMeeting(owner, myself, other1, other2, other3, handle) {
...
function startMeeting(roomId) {
    ...
    easyrtc.easyApp(roomId, myBox, videoBoxes, connectedListener);
