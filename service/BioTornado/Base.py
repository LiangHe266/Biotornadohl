...
class WebRequestHandler(tornado.web.RequestHandler):
	...
	def checkUnauthAccess(self):
		if self.__module__ == 'Login.login' :
			return True
		if self.__module__ == 'Registration.hospitalInfo' :
			return True
		if self.__module__ == 'User.patient' :
			return True	
		if self.__module__ == 'Area.list' :
			return True	
		if self.__module__ == 'mdt.query' :
			return True
		if self.__module__ == 'Menuadmin.file' :
			return True
		if self.__module__ == 'Upload.list' :
			return True		
		if self.request.method != 'GET' :
			return False
		if self.__module__ == 'report.test' :
			return True
		if self.__module__ == 'report.test2' :
			return True		
		return False
